...
# Opencode
opencode run "使用 thermo-nuclear-code-quality-review 技能审查未提交更改（只读），输出审查报告" --format json --dangerously-skip-permissions | grep '"type":"text"'

# Kimi
kimi --prompt "/skill:thermo-nuclear-code-quality-review 审查未提交更改（只读），输出审查报告"

# Cursor Agent
agent -p "使用 thermo-nuclear-code-quality-review 技能审查未提交更改（只读），输出审查报告" --yolo

# Claude Code
claude -p "/thermo-nuclear-code-quality-review 审查未提交更改（只读），输出审查报告" --dangerously-skip-permissions
```
