...
# Kimi
kimi --prompt "/skill:thermo-nuclear-code-quality-review 审查未提交更改（只读），输出审查报告到<临时文件>"

# Cursor Agent
agent -p "使用 thermo-nuclear-code-quality-review 技能审查未提交更改（只读），输出审查报告到<临时文件>" --yolo

# Claude Code
claude -p "/thermo-nuclear-code-quality-review 审查未提交更改（只读），输出审查报告到<临时文件>" --dangerously-skip-permissions
```

> 若代理不支持 CLI 调用或命令执行失败，告知用户并请求手动粘贴审查结果。
> 若用户同时选择多个编码代理，则分别请求这些编码代理审查并汇总审查结果。
> `<临时文件>`需要你来分配，避免多个代理输出到相同文件。
