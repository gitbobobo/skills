...
编写计划并保存到文档目录，由 Claude Code agent 完成计划，必须由用户主动调用。

计划中需要包含明确的验收标准。

给 Claude Code agent 的首个提示词中需要包含计划文件路径，以及“这是一个长程任务，在达到验收标准之前不得以任何理由停下来”的提示，以确保子代理不会选择性跳过部分任务。

一个关键原则：Claude Code agent 只负责实现计划，验收标准需要由你来把关，不通过则告诉子代理哪些地方还要修改，修改方向是什么。

## 前置条件

确保已安装 tmux 和 Claude Code，未安装时需要先询问用户意见：
...
- 子代理上下文窗口达到限制时，使用 `/compact` 命令压缩上下文后，让子代理继续工作
- 子代理额度达到上限时，每 10 分钟检查一次当前时间，在子代理可用时发送“继续”让子代理继续工作
